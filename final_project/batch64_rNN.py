...
def cleantwt(twt):
    #compile symbols we dont want to have
    emoj = re.compile("["
        u"\U0001F600-\U0001F64F"  #potential emojies
        u"\U0001F300-\U0001F5FF"  #all symbols
        u"\U0001F680-\U0001F6FF"  #get rid of transport stuff
        u"\U0001F1E0-\U0001F1FF"  #flag symbols for the mac/apple
        u"\U00002500-\U00002BEF"  #any chinese characters
        u"\U00002702-\U000027B0"
        u"\U000024C2-\U0001F251"
        u"\U0001f926-\U0001f937" #russian characters
    ...
    #remove everything
    twt = re.sub('RT', '', twt) #remove the retweet from a tweet
    twt = re.sub('#[A-Za-z0-9]+', '', twt) #remove any hashtags
    twt = re.sub('\\n', '', twt) #remove the linebreak
    twt = re.sub('https?:\/\/\S+', '', twt) #remove the hyperlinks
    twt = re.sub('@[\S]*', '', twt) #remove any mentions
    twt = re.sub('^[\s]+|[\s]+$', '', twt) #remove whitespaces
    twt = re.sub(emoj, '', twt) #remove the previously listed emojis
    return twt
